...
- `14-15`: legacy tuple syntax may still appear in imported x2many values.
- `16+`: prefer `Command`-based x2many writes when generating import payloads programmatically.
- `17+`: pair import/export wizards with version-correct view visibility syntax.
- `18+`: keep active-company scope explicit in bulk jobs; forward `allowed_company_ids` in Python/context where needed.
- `19`: recheck typed helper signatures and SQL-heavy export helpers against the active version references.
    ...
    with open(file_path, 'r') as f:
        ...
        for row in reader:
            ...
            if len(batch) >= batch_size:
                ...
                batch = []
                self.env.cr.commit()  # Use only in explicit import-job flows
...
3. **Batch processing** - Commit in chunks for large imports
   Pair this with `transaction-safety-patterns.md` for cron/job contexts
