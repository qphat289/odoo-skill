...
- `14-16`: binary downloads commonly use `request.make_response(...)`.
- `17-18`: prefer `Stream`-based download flows when available for attachment/binary responses.
- `19`: keep binary handling aligned with the active API version notes, especially for route type and auth semantics around download endpoints.
- Pair this file with `controller-api-patterns.md` and the active API version notes when a controller serves the file.
...
class FileUploadController(http.Controller):
    ...
    def upload_file(self, file, record_id, **kwargs):
        ...
        record = request.env['my.model'].browse(int(record_id)).exists()
        if not record:
            return request.make_json_response({'error': 'Record not found'}, status=404)
...
9. **Check access rights** - Verify permissions on upload and download
