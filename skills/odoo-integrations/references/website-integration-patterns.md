...
- `14-15`: expect more legacy website/frontend widget patterns.
- `16-18`: OWL/public component patterns can coexist with legacy public widgets.
- `17+`: public routes still need explicit controller/version checks; do not copy older `attrs`-style UI assumptions into website backoffice views.
- `19`: if the website flow depends on JSON endpoints, confirm route type and auth details against the active API version notes.
