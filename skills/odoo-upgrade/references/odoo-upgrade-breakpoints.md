...
- Keep record rules on `company_ids`; add `allowed_company_ids` only in Python/context flows that must preserve active company scope
- Prefer SQL builder and add type hints where practical

### 18.0 -> 19.0

- Review where stronger typing actually improves maintained code
- Prefer migrating remaining raw SQL toward `SQL()` builder
- Migrate SQL constraints toward `models.Constraint()`
- Re-check `res.users` group assignment patterns
- Review current frontend adjustments against the upstream `19.0` source
