...
| Chatter UI | `<chatter/>` shorthand becomes acceptable | Low - align view style intentionally |
...
- Odoo 17 commonly uses the classic `div.oe_chatter` block.
- Odoo 18 allows the shorter `<chatter/>` syntax.
- When upgrading shared XML snippets, pick one style intentionally instead of mixing both across the same module.
...
Use `allowed_company_ids` only in Python/context propagation when active company scope must travel through service logic.
