...
High-value optional folders when the feature needs them:

```text
|-- data/                   # sequences, cron, mail templates, defaults
|-- wizard/                 # TransientModel flows
|-- controllers/            # HTTP or JSON endpoints
|-- report/                 # QWeb reports and actions
|-- static/                 # JS, XML, SCSS, images
|-- i18n/                   # .pot and locale .po files
...
- If the module exposes frontend or website features, declare `assets` explicitly
- If the module is user-facing across languages, keep `i18n/{module_name}.pot` in the module

### Hook notes

- `pre_init_hook`, `post_init_hook`, and `uninstall_hook` are version-sensitive implementation points; verify the exact signature in the target version before copying examples.
- `post_load()` is process-level and should not assume normal ORM/database context.
- Do not add install hooks unless the module has a real setup or cleanup need.
...
- [ ] mixins are chosen deliberately instead of copied by habit
...
- [ ] chatter block matches the target version or uses the safe classic `oe_chatter` structure
...
### Category G - Runtime Packaging

Checklist:
- [ ] `static/description/icon.png` exists if the module should look complete in Apps
- [ ] `i18n/{module}.pot` exists for reusable/user-facing modules
- [ ] `data/` holds sequences, cron jobs, and mail templates instead of burying them in unrelated XML files
- [ ] report, controller, and wizard folders are added only when the feature actually needs them
