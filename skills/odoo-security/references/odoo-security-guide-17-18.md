...
|-----------|------|------|--------------------|
| Multi-company rule context | `company_ids` | `company_ids` in rules, `allowed_company_ids` only in Python/context | High |
...
### 1. Record rules stay on `company_ids`
...
Use `allowed_company_ids` only in Python/context propagation when service logic must preserve the active company scope.
