...
- Portal mixin and token-sharing concepts are broadly stable across supported versions.
- `14-16`: legacy portal controllers and older frontend patterns are still common.
- `17+`: pair portal routes with current controller/API notes rather than assuming older JSON/auth behavior.
- `18+`: when portal features include chatter or newer frontend pieces, align them with the active version skill.
