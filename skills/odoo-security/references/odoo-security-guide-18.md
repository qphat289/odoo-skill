...
```xml
<!-- v18: Official rule variable remains company_ids -->
...
Use `allowed_company_ids` in Python/context handling when the active company scope must be forwarded explicitly, but keep record-rule variables aligned with the documented rule context.
...
- [ ] Business logic forwards `allowed_company_ids` where active company scope matters
...
3. **Use** `company_ids` in record rule domains and `allowed_company_ids` only in Python/context where needed
