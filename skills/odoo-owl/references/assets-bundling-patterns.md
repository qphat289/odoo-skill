...
|--------|---------|---------|
| `web.assets_backend` | Backend UI | JS, SCSS, and XML templates for backend work |
| `web.assets_frontend` | Website | Public website or portal assets |
| `web.report_assets_common` | Reports | Report-only styling and assets |
| `web.assets_tests` | Tests | frontend/tour test assets |
| `point_of_sale.assets` | POS | Point of Sale |

### Version Notes

- Odoo 14-16 commonly uses `web.assets_common` and `web.assets_qweb`.
- Odoo 17+ should not rely on `web.assets_common` as a default custom-module target.
- Odoo 17+ usually places XML templates in the same asset bundle as the JS that consumes them, most often `web.assets_backend` or `web.assets_frontend`.
- When uncertain, verify the target version's `web` manifest instead of copying a bundle name from an older addon.
...
```python
{
    ...
    'assets': {
        ...
        # For v14-v16, some codebases still separate QWeb templates
        'web.assets_qweb': [
            'my_module/static/src/xml/*.xml',
        ],
    },
...
```python
{
    'assets': {
        ...
        'web.assets_frontend': [
            'my_module/static/src/frontend/js/**/*.js',
            'my_module/static/src/frontend/xml/**/*.xml',
            'my_module/static/src/frontend/scss/**/*.scss',
        ],
